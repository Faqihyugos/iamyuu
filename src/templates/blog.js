...
const Blog = ({ data: { site, allMdx }, pageContext: { pagination, categories } }) => {
  ...
  const posts = page.map(id => allMdx.edges.find(edge => edge.node.id === id)).filter(post => post !== undefined);
      ...
      <Container noVerticalPadding>
        {posts.map(({ node: post }) => (
          ...
          >
            ...
            <Link to={`/${post.fields.slug}`} aria-label={`view "${post.frontmatter.title}" article`}>
              Read Article →
            </Link>
          </div>
        ))}
        {previousPagePath || nextPagePath ? (
          <div css={css({ marginTop: '30px' })}>
            <>
              <Link to={previousPagePath} aria-label='View previous page' disabled={previousPagePath}>
                ← Previous Page
              </Link>
